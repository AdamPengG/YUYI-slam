...
void Preprocess::isaac_handler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  const double scan_duration_ms = SCAN_RATE > 0 ? 1000.0 / static_cast<double>(SCAN_RATE) : 0.0;
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    const double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y + pl_orig.points[i].z * pl_orig.points[i].z;
    if (range < blind_sqr) continue;
    ...
    added_pt.curvature = (plsize > 1) ? scan_duration_ms * static_cast<double>(i) / static_cast<double>(plsize - 1) : 0.0;
    pl_surf.points.push_back(added_pt);
  }
}
