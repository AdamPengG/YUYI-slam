...
CAMERA_ODOM_FRAME_ID = "camera_init"
CAMERA_GT_POSE_TOPIC = "/camera_pose_gt_at_image"
CAMERA_WIDTH = 1280
CAMERA_HEIGHT = 720
CAMERA_NEAR_CLIP = 0.05
# Keep the camera's visual far clip large enough for normal RGB rendering.
# Depth limiting to 8 m is enforced downstream in the exporter so Camera1
# does not "see through" distant room geometry in the viewport.
...
LIDAR_PRIM_PATH = "/World/turtlebot3_burger_ROS/base_scan/Lidar"
LIDAR_HORIZONTAL_FOV = 360.0
LIDAR_VERTICAL_FOV = 60.0
LIDAR_HORIZONTAL_RESOLUTION = 1.0
LIDAR_VERTICAL_RESOLUTION = 1.0
LIDAR_ROTATION_RATE = 20.0
TURTLEBOT_MAX_LINEAR_SPEED = 0.22
TURTLEBOT_MAX_ANGULAR_SPEED = 1.00
TURTLEBOT_MAX_ACCELERATION = 1.00
TURTLEBOT_MAX_DECELERATION = 1.50
TURTLEBOT_MAX_WHEEL_SPEED = 10.00
...
def _graph_needs_rebuild(stage) -> bool:
    ...
    read_sim_time = stage.GetPrimAtPath(f"{CAMERA_GRAPH_PATH}/ReadSimulationTime")
    read_world_pose = stage.GetPrimAtPath(f"{CAMERA_GRAPH_PATH}/ReadWorldPose")
    pose_publish = stage.GetPrimAtPath(f"{CAMERA_GRAPH_PATH}/PosePublish")
    ...
    pose_topic_attr = pose_publish.GetAttribute("inputs:topicName")
    pose_odom_frame_attr = pose_publish.GetAttribute("inputs:odomFrameId")
    pose_chassis_frame_attr = pose_publish.GetAttribute("inputs:chassisFrameId")
    pose_queue_attr = pose_publish.GetAttribute("inputs:queueSize")
    pose_prim_attr = read_world_pose.GetAttribute("inputs:prim")
    ...
    if not all(
        ...
        for attr in [
            rgb_queue_attr,
            depth_queue_attr,
            camera_info_queue_attr,
            rgb_skip_attr,
            depth_skip_attr,
            camera_info_skip_attr,
            pose_topic_attr,
            pose_odom_frame_attr,
            pose_chassis_frame_attr,
            pose_queue_attr,
            pose_prim_attr,
        ]
    ):
    ...
    if (
        ...
        return True
    if pose_topic_attr.Get() != CAMERA_GT_POSE_TOPIC:
        return True
    if pose_odom_frame_attr.Get() != CAMERA_ODOM_FRAME_ID:
        return True
    if pose_chassis_frame_attr.Get() != CAMERA_FRAME_ID:
        return True
    if pose_queue_attr.Get() != CAMERA_QUEUE_SIZE:
        return True
    try:
        pose_targets = [str(path) for path in pose_prim_attr.GetTargets()]
    except Exception:
        pose_targets = []
    if pose_targets != [CAMERA_PRIM_PATH]:
        return True
    ...
    expected_connections = [
        ...
        (pose_publish, "inputs:context", f"{CAMERA_GRAPH_PATH}/Ros2Context.outputs:context"),
        (pose_publish, "inputs:qosProfile", f"{CAMERA_GRAPH_PATH}/Ros2Qos.outputs:qosProfile"),
        (pose_publish, "inputs:execIn", f"{CAMERA_GRAPH_PATH}/RenderProduct.outputs:execOut"),
        (pose_publish, "inputs:timeStamp", f"{CAMERA_GRAPH_PATH}/ReadSimulationTime.outputs:simulationTime"),
        (pose_publish, "inputs:position", f"{CAMERA_GRAPH_PATH}/ReadWorldPose.outputs:translation"),
        (pose_publish, "inputs:orientation", f"{CAMERA_GRAPH_PATH}/ReadWorldPose.outputs:orientation"),
    ]
    ...
    og.Controller.edit(
        {"graph_path": CAMERA_GRAPH_PATH, "evaluator_name": "execution"},
        {
            og.Controller.Keys.CREATE_NODES: [
                ...
                ("ReadWorldPose", "isaacsim.core.nodes.IsaacReadWorldPose"),
                ...
                ("PosePublish", "isaacsim.ros2.bridge.ROS2PublishOdometry"),
            ],
            og.Controller.Keys.SET_VALUES: [
                ...
                ("ReadWorldPose.inputs:prim", [Sdf.Path(CAMERA_PRIM_PATH)]),
                ...
                ("PosePublish.inputs:topicName", CAMERA_GT_POSE_TOPIC),
                ("PosePublish.inputs:odomFrameId", CAMERA_ODOM_FRAME_ID),
                ("PosePublish.inputs:chassisFrameId", CAMERA_FRAME_ID),
                ("PosePublish.inputs:queueSize", CAMERA_QUEUE_SIZE),
                ("PosePublish.inputs:publishRawVelocities", True),
            ],
            og.Controller.Keys.CONNECT: [
                ...
                ("RenderProduct.outputs:execOut", "PosePublish.inputs:execIn"),
                ...
                ("Ros2Context.outputs:context", "PosePublish.inputs:context"),
                ...
                ("Ros2Qos.outputs:qosProfile", "PosePublish.inputs:qosProfile"),
                ("ReadSimulationTime.outputs:simulationTime", "PosePublish.inputs:timeStamp"),
                ("ReadWorldPose.outputs:translation", "PosePublish.inputs:position"),
                ("ReadWorldPose.outputs:orientation", "PosePublish.inputs:orientation"),
            ],
        },
    )
    return True
...
def _ensure_camera_sensor_defaults(stage) -> bool:
    camera_prim = stage.GetPrimAtPath(CAMERA_PRIM_PATH)
    if not camera_prim.IsValid():
        raise RuntimeError(f"Camera prim not found at {CAMERA_PRIM_PATH}")

    changed = False
    clipping_attr = camera_prim.GetAttribute("clippingRange")
    desired_clipping = (CAMERA_NEAR_CLIP, CAMERA_FAR_CLIP)
    if clipping_attr.IsValid():
        current_value = clipping_attr.Get()
        current_tuple = None
        if current_value is not None:
            try:
                current_tuple = (float(current_value[0]), float(current_value[1]))
            except Exception:
                current_tuple = None
        if current_tuple != desired_clipping:
            clipping_attr.Set(desired_clipping)
            changed = True
...
def _find_lidar_prim_path(stage) -> str:
    preferred_paths = [
        LIDAR_PRIM_PATH,
    ...
    for prim in stage.Traverse():
        ...
        if "lidar" in path.lower() or "lidar" in prim_type.lower():
            return path

    raise RuntimeError("Could not locate a LiDAR prim in the current TurtleBot3 stage.")


def _ensure_lidar_sensor_defaults(stage, lidar_prim_path: str) -> bool:
    lidar_prim = stage.GetPrimAtPath(lidar_prim_path)
    if not lidar_prim.IsValid():
        raise RuntimeError(f"LiDAR prim not found at {lidar_prim_path}")

    changed = False
    desired_values = {
        "horizontalFov": LIDAR_HORIZONTAL_FOV,
        "verticalFov": LIDAR_VERTICAL_FOV,
        "horizontalResolution": LIDAR_HORIZONTAL_RESOLUTION,
        "verticalResolution": LIDAR_VERTICAL_RESOLUTION,
        "rotationRate": LIDAR_ROTATION_RATE,
    }
...
def _ensure_turtlebot_motion_defaults(stage) -> bool:
    changed = False
    for prim in stage.Traverse():
        desired_attrs = {
            "inputs:maxLinearSpeed": TURTLEBOT_MAX_LINEAR_SPEED,
            "inputs:maxAngularSpeed": TURTLEBOT_MAX_ANGULAR_SPEED,
            "inputs:maxAcceleration": TURTLEBOT_MAX_ACCELERATION,
            "inputs:maxDeceleration": TURTLEBOT_MAX_DECELERATION,
            "inputs:maxWheelSpeed": TURTLEBOT_MAX_WHEEL_SPEED,
        }

        prim_changed = False
        for attr_name, desired in desired_attrs.items():
            attr = prim.GetAttribute(attr_name)
            if not attr.IsValid():
                continue
            current = attr.Get()
...
def main() -> None:
    ...
    try:
        ...
        lidar_prim_path = _find_lidar_prim_path(stage)

        camera_sensor_updated = _ensure_camera_sensor_defaults(stage)
        imu_sensor_updated = _ensure_imu_sensor_defaults(stage, imu_prim_path)
        lidar_sensor_updated = _ensure_lidar_sensor_defaults(stage, lidar_prim_path)
        motion_updated = _ensure_turtlebot_motion_defaults(stage)
        ...
        if (
            ...
            or camera_sensor_updated
            or imu_graph_rebuilt
            or imu_sensor_updated
            or lidar_sensor_updated
            or motion_updated
            or physics_updated
        ...
        print(f"camera_sensor_updated={camera_sensor_updated}", flush=True)
        print(f"camera_clipping_range=({CAMERA_NEAR_CLIP}, {CAMERA_FAR_CLIP})", flush=True)
        print(f"camera_gt_pose_topic={CAMERA_GT_POSE_TOPIC}", flush=True)
        ...
        print(f"lidar_prim_path={lidar_prim_path}", flush=True)
        print(f"imu_sensor_updated={imu_sensor_updated}", flush=True)
        print(f"lidar_sensor_updated={lidar_sensor_updated}", flush=True)
        print(f"motion_updated={motion_updated}", flush=True)
        print(f"turtlebot_max_linear_speed={TURTLEBOT_MAX_LINEAR_SPEED}", flush=True)
        print(f"turtlebot_max_angular_speed={TURTLEBOT_MAX_ANGULAR_SPEED}", flush=True)
        print(f"turtlebot_max_acceleration={TURTLEBOT_MAX_ACCELERATION}", flush=True)
        print(f"turtlebot_max_deceleration={TURTLEBOT_MAX_DECELERATION}", flush=True)
        print(f"turtlebot_max_wheel_speed={TURTLEBOT_MAX_WHEEL_SPEED}", flush=True)
        ...
        print(f"lidar_horizontal_resolution={LIDAR_HORIZONTAL_RESOLUTION}", flush=True)
        print(f"lidar_vertical_resolution={LIDAR_VERTICAL_RESOLUTION}", flush=True)
        print(f"lidar_rotation_rate={LIDAR_ROTATION_RATE}", flush=True)
