...
class OVOSemanticLidarMapPublisher(Node):
    def __init__(self) -> None:
        ...
        self._instance_bbox_lookup: dict[int, np.ndarray] = {}
        self._instance_label_lookup: dict[int, str] = {}
    ...
    def _declare_parameters(self) -> None:
        defaults = {
            ...
            "knn_k": 5,
            "match_radius_m": 0.5,
            "compact_object_bbox_margin_m": 0.25,
            "default_gray": 160,
            "semantic_only": True,
    ...
    def _load_parameters(self) -> None:
        ...
        self.compact_object_bbox_margin_m = float(self.get_parameter("compact_object_bbox_margin_m").value)
    ...
    def _timer_callback(self) -> None:
        if self.artifact_path.exists():
            ...
            if self._last_mtime_ns != mtime_ns:
                ...
                if should_load and self._load_artifact():
                    ...
                    self._last_mtime_ns = mtime_ns
        elif self._clear_needed or self._latest_projected_cloud is not None or self._cached_markers is not None:
            self._latest_projected_cloud = self._empty_cloud()
        ...
        if self._latest_cloud_msg is not None and self._snapshot_tree is not None:
            ...
            if self._latest_projected_cloud is None or self._latest_projected_from_stamp != cloud_stamp:
                projected = self._project_lidar_cloud(self._latest_cloud_msg)
                if self._cloud_has_points(projected) or self._latest_projected_cloud is None:
                    self._latest_projected_cloud = projected
                else:
                    self.get_logger().info(
                        "Skipping empty lidar semantic projection and keeping last non-empty semantic cloud."
                    )
    ...
    def _load_artifact(self) -> bool:
        try:
            ...
            with closing(np.load(self.artifact_path, allow_pickle=True)) as data:
                ...
                instance_bboxes = data.get("instance_bboxes", np.zeros((0, 6), dtype=np.float32)).astype(
                    np.float32, copy=False
                )
                ...
                instance_point_counts = data.get("instance_point_counts", np.zeros((0,), dtype=np.int32)).astype(
                    np.int32, copy=False
                )
                instance_view_counts = data.get("instance_view_counts", np.zeros((0,), dtype=np.int32)).astype(
                    np.int32, copy=False
                )
        ...
        self._instance_bbox_lookup = {
            int(ins_id): np.asarray(bbox, dtype=np.float32)
            for ins_id, bbox in zip(instance_ids.tolist(), instance_bboxes.tolist(), strict=False)
        }
        self._instance_label_lookup = {
            int(ins_id): str(label)
            for ins_id, label in zip(instance_ids.tolist(), instance_labels.tolist(), strict=False)
        }
    ...
    def _project_lidar_cloud(self, msg: PointCloud2) -> PointCloud2:
        ...
        for row in matched_rows:
            ...
            ranked_instances = [
                (int(instance_id_value), int(count))
                for instance_id_value, count in sorted(
                    zip(unique_instances.tolist(), counts.tolist(), strict=False),
            ...
            chosen_mask = None
            for candidate_instance, _count in ranked_instances:
                if self._instance_extent_allows_point(candidate_instance, pts[row]):
                    chosen_instance = int(candidate_instance)
                    chosen_mask = nbr_instance == chosen_instance
                    break
            if chosen_instance is None or chosen_mask is None:
                continue
        ...
        return point_cloud2.create_cloud(header, self._fields(), points)

    @staticmethod
    def _cloud_has_points(msg: PointCloud2 | None) -> bool:
        return msg is not None and int(getattr(msg, "width", 0)) > 0

    def _instance_extent_allows_point(self, instance_id: int, point_xyz: np.ndarray) -> bool:
        bbox = self._instance_bbox_lookup.get(int(instance_id))
        if bbox is None or bbox.size != 6:
            return True
        label = self._instance_label_lookup.get(int(instance_id), "unknown")
        if label in STRUCTURAL_LABELS:
            return True
        margin = float(self.compact_object_bbox_margin_m)
        bbox_min = bbox[:3] - margin
        bbox_max = bbox[3:] + margin
        return bool(np.all(point_xyz >= bbox_min) and np.all(point_xyz <= bbox_max))
