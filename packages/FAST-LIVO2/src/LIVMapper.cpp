...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  pubCameraPoseAtImage = this->node->create_publisher<nav_msgs::msg::Odometry>("/camera_pose_at_image", 10);
...
void LIVMapper::handleVIO() 
{
  ...
  int size_sub_map = vio_manager->visual_submap->voxel_points.size();
  visual_sub_map->reserve(size_sub_map);
  for (int i = 0; i < size_sub_map; i++) 
  {
    VisualPoint *pt = vio_manager->visual_submap->voxel_points[i];
    if (pt == nullptr) continue;
    PointType temp_map;
    temp_map.x = pt->pos_[0];
    temp_map.y = pt->pos_[1];
    temp_map.z = pt->pos_[2];
    float error = 0.0f;
    if (i < vio_manager->visual_submap->errors.size())
    {
      error = vio_manager->visual_submap->errors[i];
    }
    temp_map.intensity = 1.0f / (1.0f + std::max(error, 0.0f));
  ...
  publish_camera_pose_at_image(pubCameraPoseAtImage, LidarMeasures.last_lio_update_time);
...
void LIVMapper::run(rclcpp::Node::SharedPtr &node) 
{
  ...
  while (rclcpp::ok()) 
  {
    ...
    catch (const rclcpp::exceptions::RCLError &e)
    {
      RCLCPP_WARN(
        ...
        "ROS shutdown detected during FAST-LIVO2 loop, stopping updates and saving PCD: %s",
        e.what());
      break;
    }
    catch (const std::exception &e)
    {
      RCLCPP_ERROR(
        this->node->get_logger(),
        "Exception in FAST-LIVO2 loop, attempting to save PCD before exit: %s",
        e.what());
      break;
    }
  }
  ...
  {
    RCLCPP_ERROR(this->node->get_logger(), "Failed to save PCD on shutdown: %s", e.what());
  }
}
...
{
  if (!rclcpp::ok() || !pubLaserCloudFullRes || pcl_w_wait_pub->empty()) return;
...
{
  if (!rclcpp::ok() || !pubLaserCloudEffect)
  {
    return;
  }
...
void LIVMapper::publish_camera_pose_at_image(
  const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubCameraPoseAtImage,
  double image_time_sec)
{
  if (!rclcpp::ok() || !pubCameraPoseAtImage || !vio_manager || !vio_manager->new_frame_)
  {
    return;
  }

  const SE3<double> T_w_c = vio_manager->new_frame_->T_f_w_.inverse();
  const Eigen::Quaterniond q_w_c(T_w_c.rotationMatrix());
  nav_msgs::msg::Odometry camera_pose_msg;
  camera_pose_msg.header.frame_id = "camera_init";
  camera_pose_msg.child_frame_id = "camera_at_image";
  camera_pose_msg.header.stamp = sec2Stamp(image_time_sec);
  camera_pose_msg.pose.pose.position.x = T_w_c.translation().x();
  camera_pose_msg.pose.pose.position.y = T_w_c.translation().y();
  camera_pose_msg.pose.pose.position.z = T_w_c.translation().z();
  camera_pose_msg.pose.pose.orientation.x = q_w_c.x();
  camera_pose_msg.pose.pose.orientation.y = q_w_c.y();
  camera_pose_msg.pose.pose.orientation.z = q_w_c.z();
  camera_pose_msg.pose.pose.orientation.w = q_w_c.w();
  pubCameraPoseAtImage->publish(camera_pose_msg);
}

void LIVMapper::publish_mavros(const rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr &mavros_pose_publisher)
{
  if (!rclcpp::ok() || !mavros_pose_publisher)
  {
    return;
  }
