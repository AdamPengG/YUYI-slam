...
def generate_launch_description():
    ...
    #Load parameters
    avia_config_cmd = os.path.join(config_file_dir, "avia_isaac.yaml")
    ...
    use_image_republish_arg = DeclareLaunchArgument(
        "use_image_republish",
        default_value="False",
        description="Whether to launch image_transport republish for compressed camera input.",
    )

    rviz_config_arg = DeclareLaunchArgument(
        "rviz_config_file",
        default_value=default_rviz_config_file,
        description="Full path to the RViz config file to use when launching RViz2.",
    )
